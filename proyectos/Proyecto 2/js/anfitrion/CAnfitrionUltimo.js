...
function obtenerEstadoAnfitrion() {
  const data = localStorage.getItem("estadoAnfitrion") || sessionStorage.getItem("estadoAnfitrion");
  return data ? JSON.parse(data) : null;
}

function limpiarEstadoAnfitrion() {
  localStorage.removeItem("estadoAnfitrion");
  sessionStorage.removeItem("estadoAnfitrion");
  localStorage.removeItem("anfitrion_ubicacion");
  sessionStorage.removeItem("anfitrion_ubicacion");
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const estado = obtenerEstadoAnfitrion();
    if (!estado?.ubicacion) {
        mostrarError("Falta la ubicación del anfitrión");
        return;
    }

    // Tipos de espacio (según tu otro archivo)
    const tiposEspacio = estado?.anfitrion?.tiposEspacio || [];
    if (!tiposEspacio.length) {
        mostrarError("Falta seleccionar tipos de espacio");
        return;
    }

    // Datos del formulario (ajustá ids si tus inputs se llaman distinto)
    const usuario = (document.getElementById("email")?.value || "").trim();      // o username
    ...
    const payload = {
        usuario,                 // va a AdminCuenta.usuario
        password,                // lo hasheamos en SQL (mismo sistema que login admin)
        telefono: { codigoPais, numero },
        ubicacion: estado.ubicacion,      // provincia/canton/distrito/direccion/googleMaps
        tiposEspacio               // array
    };

    try {
        const resp = await fetch("http://localhost:3001/api/auth/register/anfitrion", {
        method: "POST",
        ...
        if (!resp.ok || data.ok === false) throw new Error(data.error || "No se pudo registrar anfitrión");

        mostrarExito("¡Anfitrión creado!");
        limpiarEstadoAnfitrion();

        setTimeout(() => window.location.href = "../../InicioSesion.html", 1200);
    } catch (err) {
